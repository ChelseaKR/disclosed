"""Google Analytics 4 on the published site, and only there (ADR 0011).
...
script that may load gtag.js. :func:`disclosed.site.build` puts that script in the head of every
page only when it is given an ID, and without one the build is byte-for-byte what it was, with no
script, no privacy page and no analytics sentence anywhere.
...
2. the page is served from ``https://chelseakr.github.io/disclosed/``. A local build, the
   Lighthouse job on 127.0.0.1 and a fork's Pages site all fail this, so none of them ever sends
   a hit to this property;
...
Not from the message catalog: the ID, the host, the key and the configuration are data. The
words the footer control shows are prose, and those are read from the catalog like every other
sentence the site renders.
"""
...
#: The web stream of GA4 property 554861135 (14-month retention, Google signals off). Public: it
#: is in every page served. ``disclosed site`` passes it by default; ``--ga4-id ""`` turns
#: analytics off for a build.
GA4_MEASUREMENT_ID: Final[str] = "G-5SE0M4LS60"

#: Where the published site is served. The script compares the page's own address with these at
#: runtime, rather than with the ``origin`` a build was given, so a fork deployed elsewhere with
#: the committed ID still sends nothing.
PUBLISHED_HOST: Final[str] = "chelseakr.github.io"
PUBLISHED_PATH: Final[str] = "/disclosed/"
...
OPT_OUT_KEY: Final[str] = "disclosed:analytics-opt-out"
...
#: The footer control's words, each a catalog key under ``analytics.``.
...
def loader(measurement_id: str, catalog: Catalog) -> str:
    """The ``<script id="analytics">`` block for the head of every page.

    Raises ``ValueError`` on a malformed ID, the same refusal :func:`measurement_id_or_none`
    makes, so no caller can render a tag for an ID nothing would accept.
    """
    ...
    return _TEMPLATE % {
        ...
        "messages": _js({name: catalog.text(f"analytics.{name}") for name in _MESSAGES}),
    }
