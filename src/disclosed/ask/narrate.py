...
- Drift records: state the source, the two snapshots, the field, and the direction word the record
  carries ("gained", "lost", or "unchanged" -- unchanged means the rate itself did not move, not
  that nothing about the record changed). Never infer direction from the counts, never compare
  IPEDS with the College Scorecard, and never call an unmeasured field unchanged -- unmeasured and
  unchanged are different facts and the record says which one applies. rate_change is a change in
  the share of applicable institutions reporting; express it in percentage points.
